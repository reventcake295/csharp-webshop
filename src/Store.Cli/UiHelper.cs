...
public class UiHelper
{
    ...
    internal static bool AskQuestion<T>(string questId, out T answer, T defaultValue, List<T>? choices = null, bool optional = false, bool hidden = false, int min = 0, int max = 0, bool caseSensitive = true)
    {
        ...
        answer = defaultValue;
        for (int i = 0; i < App.GetSettings().MaxInputLoop; i++)
        {
            T responseCast;
            Console.Write($"{LangHandler.GetSpecificOrDefaultString(questId, LangHandler.StringType.Question)}: ");
            string? response = !hidden ? Console.ReadLine() : AskSecret();
            if (string.IsNullOrWhiteSpace(response))
            {
                if (optional) return true;
                Console.WriteLine(LangHandler.GetSpecificOrDefaultString(questId, LangHandler.StringType.QuestionEmpty));
                continue;
            }
            ...
            {
                Console.WriteLine(LangHandler.GetSpecificOrDefaultString(questId, LangHandler.StringType.QuestionWrong));
                continue;
            }
        ...
        Console.WriteLine(LangHandler.GetSpecificOrDefaultString(questId, LangHandler.StringType.QuestionMaxLoop));
        return false;
    }
    ...
    internal static void DisplayResult(string questId, LangHandler.StringType stringType) => Console.WriteLine(LangHandler.GetSpecificOrDefaultString(questId, stringType));
    
    internal static void DisplayOption(string key, string value) => Console.WriteLine($"    {key}: {LangHandler.GetLangString(value)}");
    ...
    {
        Console.WriteLine("\n" + LangHandler.GetSpecificOrDefaultString(menuId, LangHandler.StringType.Header) + "\n");
